...
export async function GET(request: NextRequest) {
  ...
  const directoryString = join(getInitialDirectory(), "downloads", "/");
      ...
      getContent.map(async (file) => {
        ...
        if (stat.isDirectory()) {
          return {
            ...
            type: "directory",
            children: [],
            url: join("/downloads", file),
            download: join("/api/downloads", file + "?zip=true"),
            date: stat.mtime,
          };
        } else {
          return {
            ...
            size: stat.size,
            url: join("/downloads", file),
            download: join("/api/downloads", file),
            date: stat.mtime,
          };
        }
      })
